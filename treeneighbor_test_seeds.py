...
class TreeGNNNode(nn.Module):
    ...
    def _compute_alt_edge_index(self, batched_data: Data, layer_idx: int | None = None):
        ...
        if mode in ['p-egp', 'ep-egp', 'f-egp']:
            base_edges = self._batched_cayley_edge_index(
                batched_data, counts, offsets, device
            )
...
def main():
    ...
    for seed in seeds:
        ...
        print('Experiments for p-egp (all expander layers)')
        p_egp_score = 0.0
        for _ in range(NUM_ITER):
        ...
        ep_egp_score = 0.0
        for _ in range(NUM_ITER):
            ep_egp_score += run_experiment(ep_egp_model, train_list, val_list, test_list, train_loader, val_loader, test_loader, criterion, transform_name='EP-EGP')
        ...
        f_egp_score = 0.0
        for _ in range(NUM_ITER):
            f_egp_score += run_experiment(f_egp_model, train_list, val_list, test_list, train_loader, val_loader, test_loader, criterion, transform_name='F-EGP')
