...
def save_curves_json(
    ...
    *,
    ...
    payload = {
        "meta": meta or {},
        "curves": {
            "train_acc": train_curve,
            "val_acc": val_curve,
            "test_acc": test_curve,
        },
        "best": {
            "epoch": int(best_val_epoch),
            "val_acc": float(val_curve[best_val_epoch]),
            "test_acc": float(test_curve[best_val_epoch]),
        },
        "num_epochs": len(train_curve),
    }
...
def run_experiment(
    ...
    if log_dir is not None:
        ...
        out_path = os.path.join(log_dir, f"{run_name}.json")
        ...
        print(f"[LOG] curves written to {out_path}")
